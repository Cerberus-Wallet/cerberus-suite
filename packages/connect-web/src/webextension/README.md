...
| WebExtension |   ✓    |    ✓    |                |                 |        |  ✓   | Requires Trezor Bridge on platforms not supporting WebUSB.              |
| Web (Bridge) |   ✓    |    ✓    |                |                 |   ✓    |  ✓   | Trezor Bridge is required for Safari and where WebUSB is not supported. |

-   "✓" indicates environments where the feature is supported.
-   Empty cells indicate environments where the feature is not supported or not applicable.
-   Notes provide additional details about support and requirements.
