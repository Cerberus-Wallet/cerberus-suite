...
import { TorStatus } from '@cerberus/suite-desktop-api';
...
export const initSentry = ({ mainThreadEmitter, store }: InitSentryParams) => {
    ...
    const transportOptions = {
        beforeSend: () => {
            if (store.getTorSettings().running && torStatus !== TorStatus.Enabled) {
                ...
                // Queue can be inspected in @cerberus/suite-desktop/sentry/queue folder.
                return 'queue';
            }
    ...
    // Sentry ignore userPath change by environment so even in local build it uses @cerberus/suite-desktop/sentry folder.
    init(sentryConfig);
};
