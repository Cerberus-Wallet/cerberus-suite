...
exports.initCerberusConnect = sender => {
    ...
    CerberusConnect.init({
        ...
        .then(() => {
            sender.send('cerberus-connect', 'CerberusConnect is ready!');
        })
        .catch(error => {
            sender.send('cerberus-connect', `CerberusConnect init error:${error}`);
        });
};

exports.callCerberusConnect = (sender, message) => {
    const { method, params } = message;
    CerberusConnect[method](params).then(response => {
        sender.send('cerberus-connect', response);
    });
};
